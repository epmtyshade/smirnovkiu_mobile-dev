import 'dart:async'; // Для FutureOr
...
class CommandRunner {
  ...
  // Обновлённый метод parse с проверками и исключениями
  ArgResults parse(List<String> input) {
    ...
    // Проверка: нельзя передавать несколько команд
    if (results.command != null &&
        input.isNotEmpty &&
    ...
    while (i < input.length) {
      if (input[i].startsWith('-')) {
        ...
        // Проверка: команда может иметь только один позиционный аргумент
...
// Вспомогательная функция для удаления префиксов -- или -
